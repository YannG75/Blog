...
if (isset($_POST['save'])) {
    ...
    if (!empty($_FILES['image']['tmp_name'])) {
        ...
        foreach ($_POST['category_id'] as $categoryId) {
            $query_categories = $db->prepare('INSERT INTO article_category (article_id, category_id) VALUES (?,?)');
...
if (isset($_POST['update'])) {
    ...
    if ($_FILES['image']['error'] == 0) {
        $queryDel = $db->prepare('SELECT image FROM article WHERE id = ?');
        $queryDel->execute([
            $_POST['id']
        ]);
        ...
        $queryImgDel = $db->prepare('UPDATE article SET image = :image WHERE id = :id');
        $queryImgDel->execute([
...
if (isset($_GET['article_id']) && isset($_GET['action']) && $_GET['action'] == 'edit') {
    ...
    $query = $db->prepare('SELECT a.*, ac.category_id 
        ...
        <section class="col-9">
            ...
            <form action="article-form.php" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" multiple name="category_id[]" id="category_id">
                        <?php
                        ...
                        <?php foreach ($categories as $key => $category) : ?>
                            ...
                            <?php $selectedCategories = $db->prepare('SELECT * FROM article_category WHERE article_id = ? AND category_id = ?');
                            $selectedCategories->execute(array($_GET['article_id'], $category['id']));
                            $SelectedCat = $selectedCategories->fetch(); ?>

                            <option value="<?= $category['id']; ?>" <?= isset($_GET['article_id']) && $SelectedCat ? 'selected' : ''; ?>>
                                <?= $category['name']; ?>
                            </option>
                        <?php endforeach; ?>
